...
- Turkish medication reminder for adults, iOS and Android.
...
- The default sync server is `https://api.mytepeapi.com.tr` (custom domain on the existing `rutin-api` Cloudflare Worker with Turso). The previous `https://rutin-api.tepe-aykut05.workers.dev` address times out during TLS on the tested network. Saved previous-cloud, HTTP and private-network addresses migrate to the custom domain. Preserve account isolation and local snapshots; migrate stored tokens only after verifying the same user on the new origin. Never use arbitrary remote fallback servers.
- Keep `api.mytepeapi.com.tr` in the API project's Wrangler custom-domain routes so subsequent deployments retain it. API source lives in the sibling `../rutin-api` project; `worker/index.js` in this project is the protected prototype runtime, not the sync API.
- Treat DNS, TLS, HTTP and authenticated sync checks separately. A successful health check alone is not an end-to-end sync test. Do not automatically replay timed-out login, recovery or sync writes; the server may have committed them. Preserve credentials after a failed logout.
...
- The notification gate also waits for the launch account check (`accountChecked`): on a signed-in phone `bindAccount` rewrote doses from a stale render and dropped taps made meanwhile, so a cold-start "İlaç İçildi" recorded nothing. The emulator has no account and release builds accept only the HTTPS sync server, so this path is verified manually on the phone, not by E2E.
- A dose added or edited after its early-reminder moment (lead time) but before its dose time gets its main alert at the dose time (`buildMedicationSchedule`, based on `updatedAt`); previously the heads-up was skipped and the first alert was a "Henüz İçilmedi" repeat. Tapping the notification body only opens the app; only the action buttons record a dose.
- Tapping a medicine notification body switches to Bugün, whose dose card offers Al / Ertele / Atla. The in-app top banner was removed in v1.2.7 (owner decision 2026-09-26): Android's own heads-up already shows the reminder in the foreground, so the banner duplicated it and stayed pinned over the app. Appointment reminders offer 1 h / 3 h snooze (opening the app); resync keeps snoozes of active appointments (`appt-snooze-<id>-…`) and drops those of completed or deleted ones. Deleting a medicine dismisses its already shown notifications (`dismissDoseNotifications`).
...
- Product direction (owner decision on 2026-09-25): publish on Google Play (App Store later if needed); the owner controls optional features and updates centrally, users may only hide what the owner leaves enabled. Planned as read-only `/api/config` on `rutin-api` with bundled safe defaults; medication core (medicines, planning, history, notifications) never depends on a flag. Before Play submission the GitHub-Releases self-updater must be disabled in the Play build (Play policy forbids self-updating).
- Off-device backup (`mobile-app/src/backup.ts`, format v3) carries doses, the full persisted settings object including appointments, learned medicines and language. Restore replaces local data after a confirmation, validates every field (bad files change nothing), keeps current appointments for v1/v2 files, refuses a backup of another account while signed in, and is blocked during a sync or account operation. Restored active doses get `updatedAt = now` so they win the next sync. The JSON file is not encrypted.
- Cloud master switch (`cloudSyncEnabled`, stored in the sync config, default on): off = local mode. Launch skips the server account check so notification actions never wait on the network; manual, automatic and profile sync send nothing; the stored token, sync code and bound account stay, and switching back on re-verifies the account and runs one merge. Toggling bumps `cloudEpochRef`; a sync reply that arrives after a toggle is dropped (the server may already have stored the write). Sync replies now merge into the latest local doses (`dosesRef.current`), not the list captured when the request started. Barcode server lookup and the update check are not covered by this switch. Backup restore checks the bound account (`boundAccountId`) when no session is verified.
- Owner decision on 2026-09-26: server sync and the diagnostics log screen are parked for a later update. `CLOUD_SYNC_AVAILABLE = false` in `mobile-app/App.tsx` hides the cloud switch, account and auto-sync UI and forces local mode (`cloudActive`) without overwriting the stored `cloudSyncEnabled` choice or the account link; `DIAGNOSTICS_AVAILABLE = false` hides the "Hata & Tanılama Günlüğü" menu entry while the logger keeps recording. These are constants rather than commented-out JSX because the blocks contain nested `{/* */}` comments. Update check (GitHub, then `/api/version`) and the barcode catalog lookup still use the network.
- Settings simplification (v1.2.7, owner decision 2026-09-26): "Bildirim ve Ses" and "Hatırlatıcı & Erteleme" share the **Hatırlatmalar** page (`notifications` subpage); "Gizlilik" and "Uygulama Deneyimi" share **Görünüm** (`experience`); the sync entry is titled **Yedekleme** while `CLOUD_SYNC_AVAILABLE` is false. The **Alarm Güvenilirliği** page has no switches: the app cannot read battery-optimization or exact-alarm state, so it only offers "Ayarları Aç" buttons (Android only) and a test alarm. The old `batteryExemptionEnabled`, `exactAlarmEnabled`, `autoRescheduleOnBoot` and `wakeScreenOnAlarm` flags were removed; they claimed protection they did not provide. Prefer plain Turkish over terms like Doze, Exact Alarm, Boot, haptics, JSON in user-facing text.
