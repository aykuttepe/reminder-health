...
- Off-device backup (`mobile-app/src/backup.ts`, format v3) carries doses, the full persisted settings object including appointments, learned medicines and language. Restore replaces local data after a confirmation, validates every field (bad files change nothing), keeps this phone's permission toggles (`exactAlarmEnabled`, `batteryExemptionEnabled`), keeps current appointments for v1/v2 files, refuses a backup of another account while signed in, and is blocked during a sync or account operation. Restored active doses get `updatedAt = now` so they win the next sync. The JSON file is not encrypted.
